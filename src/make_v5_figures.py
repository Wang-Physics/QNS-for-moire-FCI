"""End-page 9/27-cell QNS summaries and 27-cell observables for report v5."""
...
RELEASE5 = ROOT / "result/release_data/v5"
NINE = RELEASE5 / "nine"
QNS27 = RELEASE5 / "qns27"
OBS27 = QNS27 / "diagnostics"
COORD27 = QNS27 / "frozen5_coordinate"
...
def plot_timing(axis, values: np.ndarray, cells: int) -> None:
    ...
    axis.set_xticks(np.arange(4), [ed_label, r"full $M$", r"v5 $P_\Gamma$" + "\n" + r"$[M]$",
                                          "v5 Outer\nper sector"])
...
def coordinate27(filling: str, branch: str):
    prefix = "nu1of3" if filling == "1/3" else "nu2of3"
    suffix = {"full": "full_m", "gamma": "v4_gamma",
              "outer0": "v4_outer_c3_p0", "outer1": "v4_outer_c3_p1",
              "outer2": "v4_outer_c3_p2"}[branch]
    return np.load(COORD27 / f"{prefix}_{suffix}.npz")
...
def figure6() -> None:
    ...
    handles = [Line2D([0], [0], color=".22", marker="o", mfc=MAGENTA, mec="white", label="multiband ED"),
               ...
               Line2D([0], [0], color=RED, label=r"v5 $P_\Gamma[M]$"),
...
def observable_scales():
    ...
    # Use the exact Figure 3 ED color ranges. QNS values outside these ranges
    # intentionally saturate, making excess amplitude directly visible.
    density = []
    for filling in ("1/3", "2/3"):
        for branch in ("full", "gamma", "outer0", "outer1", "outer2"):
            record = coordinate27(filling, branch)
            density.append(record["charge_density_over_mean"].ravel())
    nk1_norm = Normalize(0.0, 0.8)
    nk5_norm = Normalize(0.0, 0.8)
    sq_norm = Normalize(0.0, 1.1)
...
def plot_observable_figure(name: str, rows: list[tuple[str, str, str]]) -> None:
    ...
    for row, (filling, branch, label) in enumerate(rows):
        ...
        coordinate = coordinate27(filling, branch)
        # Display both physical projections without trace rescaling. Their
        # sums remain N_e W_1 and the measured five-band trace T_5.
        nk1 = np.asarray(record["momentum_occupation_band1_raw"])
        nk5 = np.asarray(record["momentum_occupation_first_five_raw"])
        nk5_raw_sum = float(np.sum(nk5))
        nk5_raw_sum_sem = float(
            record["momentum_occupation_first_five_raw_sum_sem"]
        )
        sq = np.asarray(coordinate["charge_structure_factor"])
        record_k = np.asarray(record["momentum_k_points"])
        record_q = np.asarray(coordinate["structure_q_vectors"])
        if not np.allclose(record_k, first_bz_momenta(), rtol=0.0, atol=1.0e-12):
            raise ValueError(f"{filling} {branch}: n(k) does not use the ED mesh")
        if not np.allclose(record_q, first_bz_momenta(), rtol=0.0, atol=1.0e-12):
            raise ValueError(f"{filling} {branch}: S(q) does not use the ED mesh")
        axis = axes[row, 0]
        ...
        artists[0] = axis.scatter(record_k[:, 0], record_k[:, 1], c=nk5, cmap="turbo",
                                  norm=nk5_norm,
                                  s=22 + 72*np.clip(nk5, 0, None)/nk5_norm.vmax,
                                  edgecolor="white", linewidth=.45, zorder=5)
        set_hex_limits(axis, model)
        annotate_hex(axis, model)
        axis.set_title(
            rf"$n_{{\rm tot}}(\mathbf{{k}}):\,"
            rf"{value_with_sem(nk5_raw_sum, nk5_raw_sum_sem)}$", pad=4
        )
        axis = axes[row, 1]
        hex_contour(axis, model, points, berry_values, "RdBu_r", berry_norm, alpha=.34)
        artists[1] = axis.scatter(record_k[:, 0], record_k[:, 1], c=nk1, cmap="turbo",
                                  norm=nk1_norm,
                                  s=22 + 72*np.clip(nk1, 0, None)/nk1_norm.vmax,
                                  edgecolor="white", linewidth=.45, zorder=5)
        set_hex_limits(axis, model)
        annotate_hex(axis, model)
        ...
        artists[2] = axis.scatter(record_q[:, 0], record_q[:, 1], c=sq, cmap="cool",
                                  norm=sq_norm,
                                  s=8 + 34*np.clip(sq, 0, None)/sq_norm.vmax,
                                  edgecolor="white", linewidth=.45, zorder=5)
        set_hex_limits(axis, model)
        annotate_hex(axis, model)
        axis.set_title(r"$S(\mathbf{q})$", pad=4)
        axis = axes[row, 3]
        x = np.asarray(coordinate["density_x_fraction"])
        y = np.asarray(coordinate["density_y_fraction"])
        density = np.tile(
            np.asarray(coordinate["charge_density_over_mean"]), (2, 2)
        )
    ...
    for column, (artist, label) in enumerate(zip(
        ...
        if column in (0, 1):
            colorbar.set_ticks(np.linspace(0.0, 0.8, 5))
        elif column == 2:
            colorbar.set_ticks([0.0, 0.3, 0.6, 0.9, 1.1])
        else:
